...
def get_exp_metrics(comp_descs, compare_by="model", all_results=False):
    ...
    if compare_by == "model":
        ...
        sorted_exps = [(f"{comp_descs["parses"][int(i/len(comp_descs["parses"]))]}-{comp_descs["params"][i%len(comp_descs["parses"])]}", []) for i in range(len(comp_descs["parses"]) * len(comp_descs["params"]))]
